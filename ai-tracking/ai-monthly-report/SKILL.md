...
### Case A: Claude Code 환경 — 월별 분리 파일 읽기

**ai-activity-tracker hook이 누적한 표준 경로**: `~/.claude/ai-sessions/YYYY-MM.json`

```bash
# 단월 (예: 2026년 4월)
[ -s ~/.claude/ai-sessions/2026-04.json ] && cat ~/.claude/ai-sessions/2026-04.json | jq '.'

# 분기 (Q1: 1~3월) - 3개 파일 합치기
jq -s 'add' ~/.claude/ai-sessions/2026-0{1,2,3}.json

# 반기 (1~6월) - 6개 파일 합치기
jq -s 'add' ~/.claude/ai-sessions/2026-0{1,2,3,4,5,6}.json
```
...
# 단월
path = Path.home() / ".claude" / "ai-sessions" / "2026-04.json"
sessions = json.load(open(path)) if path.exists() else []

# 분기/반기 - glob으로 합치기
sessions_dir = Path.home() / ".claude" / "ai-sessions"
files = sorted(sessions_dir.glob("2026-0[1-3].json"))  # Q1
all_sessions = []
for f in files:
    all_sessions.extend(json.load(open(f)))
```

**구버전 단일 파일과의 호환** (마이그레이션 시):
```bash
# 구버전 ~/.claude/ai-sessions.json 이 있으면 월별로 분리
if [ -f ~/.claude/ai-sessions.json ]; then
  python3 -c "
import json, os
from pathlib import Path
old = json.load(open(os.path.expanduser('~/.claude/ai-sessions.json')))
by_month = {}
for s in old:
    m = s['date'][:7]
    by_month.setdefault(m, []).append(s)
out_dir = Path.home() / '.claude' / 'ai-sessions'
out_dir.mkdir(parents=True, exist_ok=True)
for m, items in by_month.items():
    fp = out_dir / f'{m}.json'
    existing = json.load(open(fp)) if fp.exists() else []
    json.dump(existing + items, open(fp, 'w'), ensure_ascii=False, indent=2)
print(f'✓ Migrated {len(old)} sessions to {len(by_month)} monthly files')
"
fi
```
...
- **Code**: `~/.claude/ai-sessions/YYYY-MM.json` 읽기 (단월/분기/반기에 따라 1~6개 파일 합산, 위 Case A)
...
<body>
  ...
  <script type="text/babel" data-presets="react">
    ...
    // ── 실제 세션 데이터 (~/.claude/ai-sessions/YYYY-MM.json 1~6개 합산해 로드한 값 인라인) ──
